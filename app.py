...
def load_google_sheet():
    ...
    creds = Credentials.from_service_account_file("data-vis-453117-0bdbe9043c4c.json", scopes=scope)
...
df = st.session_state.df  # Use cached data
...
# Count birdstrikes per flight phase per year
...
# Initialize session state variables
    if 'selected_graph' not in st.session_state:
        st.session_state.selected_graph = np.random.choice([1, 2])  # Pick randomly once
        st.session_state.first_show = False
        st.session_state.second_show = False
        st.session_state.start_time = None  # Store timestamp for first button click
        st.session_state.mid_time = None  # Store timestamp for second button click
        st.session_state.end_time = None  # Store timestamp when second graph is displayed

    # Button to show the first graph
    if st.button("Show Graph"):
        st.session_state.first_show = True
        st.session_state.start_time = time.time()  # Store timestamp of first click

    # Display the first graph if "Show Graph" was clicked
    if st.session_state.first_show:
        ...
        # Button to show the second graph (only appears after first graph is shown)
        if st.button("I answered your question"):
            st.session_state.second_show = True
            st.session_state.mid_time = time.time()  # Store timestamp of second click

    # Display the second graph if "I answered your question" was clicked
    if st.session_state.second_show:
        if st.session_state.selected_graph == 1:
            st.pyplot(fig2)  # Show the one that wasn't selected first
        else:
            st.pyplot(fig1)  # Show the other graph
